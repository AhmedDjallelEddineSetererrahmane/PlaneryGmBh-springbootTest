...
public class MyController {
    @PostMapping(value = "/multiplication", consumes = "application/json", produces = "application/json")
    public Response multiplyTwoValue(@Valid @RequestBody Request request){
        return new Response(request.getValueA(),request.getValueB(),request.getValueA()*request.getValueB());
    }
}
